# https://github.com/xuebinqin/DIS/blob/996c949/IS-Net/models/isnet.py
import torch
...
class ISNet:
    def __init__(self, device):
        path = "data/isnet/isnet-general-use.pth"
        self.net = ISNetDIS()
    ...
    def __call__(self, image):
        _, _, h, w = image.shape
        x = F.interpolate(image, self.input_size, mode='bilinear', align_corners=False)
        x = normalize(x, [0.5,0.5,0.5], [1.0,1.0,1.0])
        x = self.net(x)
        x = F.interpolate(x[0][0], (h, w), mode='bilinear', align_corners=False)

        ma = x.amax(dim=(-2, -1), keepdim=True)
        mi = x.amin(dim=(-2, -1), keepdim=True)
        x = (x - mi) / (ma - mi)
        return x
